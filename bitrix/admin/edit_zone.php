...
if(count($PROP)>0):
	...
	foreach($PROP as $prop_code=>$prop_fields):
			...
			<td>
				...
				<?else:
					...
					foreach($prop_values as $key => $value):
						...
						?>
						<script src="http://api-maps.yandex.ru/2.0/?load=package.full,util.json&lang=ru-RU&wizard=sebekon.deliverycalc" type="text/javascript"></script>
						...
						<script type="text/javascript">
							...
							function saveGeometry (coords) {
								$('#PROP_<?=$prop_fields["ID"]."_".$key?>').val(stringify(coords));
							}
						</script>
		...
		if($prop_code == "ZONE_COLOR"):
			...
			<script language="JavaScript" >
				$(function(){
					var input = $('#tr_PROPERTY_<?=$prop_fields["ID"];?>').find('input:first');
					if(input.val()==''){
						input.val('#ff0000');
					}
					$('<div id="farbtastic<?=$prop_fields["ID"]?>"></div>').insertAfter(input);
					
					$('#farbtastic<?=$prop_fields["ID"]?>').farbtastic(input);
				});
			</script>
		<?endif;
