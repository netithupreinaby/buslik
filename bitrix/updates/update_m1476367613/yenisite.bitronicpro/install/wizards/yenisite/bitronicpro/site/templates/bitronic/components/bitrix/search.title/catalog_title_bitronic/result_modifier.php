			...
			if(CModule::IncludeModule("catalog")){
				foreach($arParams[PRICE_CODE] as $prid){
					...
					if ($ar_res = $db_res->Fetch())
					{
                        ...
                        //$arResult["SEARCH"][$i][DISCOUNTS][$prid] = $arDiscounts;
                        
                        if($arDiscounts[0]["VALUE"] > 0)
                        {
                            if($arDiscounts[0]["VALUE_TYPE"] == "P"){
                                $ar_res["PRICE"] = $ar_res["PRICE"] - $arDiscounts[0]["VALUE"]*$ar_res["PRICE"]/100;
                            }
                            else{
                                $ar_res["PRICE"] = $ar_res["PRICE"] - $arDiscounts[0]["VALUE"];
                            }
                        }
                        
                    	$arResult["SEARCH"][$i][PRICES][$prid] = CurrencyFormat($ar_res["PRICE"], $ar_res["CURRENCY"]);    
		...
		/* preview start */
			if(CModule::IncludeModule('iblock')){
				...
				if($ar_res){
				    ...
				    {
					    $prop = &$props[$pid];
					     CIBlockFormatProperties::GetDisplayValue($arResult, $prop, «search_out»);
					     $arResult["SEARCH"][$i]["PROPERTIES"] [$pid]= CIBlockFormatProperties::GetDisplayValue( $arResult["SEARCH"][$i] , $prop, "catalog_out");
				    }
				}
			}
		/* preview end */
