...
if( $obCache -> InitCache($arParams['CACHE_TIME'], "yenisite_accessories_{$arParams['ELEMENT_ID']}") && false)
	...
	if(is_array($arParams['ACCESSORIES_PROPS']['PROPERTY']))
	{
		foreach($arParams['ACCESSORIES_PROPS']['PROPERTY'] as &$arProperty)
		{
			...
			if($arProperty['VALUE'])
			{
				...
				$arFilter = Array('IBLOCK_ID' => $arProperty['IBLOCK_ID'], $prop_code => $arProperty['VALUE'], '!ID' => $arParams['ELEMENT_ID']) ;
