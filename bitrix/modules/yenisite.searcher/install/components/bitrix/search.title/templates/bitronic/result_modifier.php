...
if($_REQUEST["ys_st_ajax_call"] === "y"){

	$sid = $_REQUEST['site_id'];
	$sid .= ""; //security scanner hack
	define("SITE_ID", $sid);
	...
	/*if(!check_bitrix_sessid()) die('check_bitrix_sessid');*/

	/*ADD IN BASKET*/
	
	if($_REQUEST["search_add2basket"]  === "y" )
	{
	...
	else
	{
		...
		{
			CPHPCache::Clean("ys_st_params_".SITE_ID, "/");
		}elseif($save_param->InitCache($lifetime, "ys_st_params_".SITE_ID, "/")) {
	...
	if($save_param->InitCache($lifetime, "ys_st_params_".SITE_ID, "/")) :
		$vars = $save_param->GetVars();
...
if($_REQUEST["ys_st_ajax_call"] === "y"){
	...
	$query = ltrim($_REQUEST["q"]);
		...
		if(!isset($arParams["PAGE_2"]) || strlen($arParams["PAGE_2"])<=0)
			$arParams["PAGE_2"] = "#SITE_DIR#search/catalog.php";
	if(CModule::IncludeModule("search")){	
		CUtil::decodeURIComponent($query);

		$categoryfilter = $_REQUEST["cat"];
		if(!isset($categoryfilter))
			$categoryfilter = $_REQUEST["search_category"];
		if(!isset($categoryfilter))
			$categoryfilter = "all";
	...
	{	
		...
		if(!empty($arResult["CATEGORIES"]) /*&& $categoryfilter == "all"*/)
		{
...
foreach($arResult["SEARCH"] as $i=>$arItem)
{
	switch($arItem["MODULE_ID"])
	{
		...
		/* price start */
			...
			{
				foreach($arParams["PRICE_CODE"] as $prid){
					$arPriceCode[$prid] = "";
				}
		
				$arPrices = CMarketPrice::GetItemPriceValues($arItem["ITEM_ID"], $arPriceCode);
				...
				{						
					if(isset($arResult["SEARCH"][$i]["MIN_PRICE"])){
						if($arResult["SEARCH"][$i]["MIN_PRICE"] > $arPrices[$prid])
							$arResult["SEARCH"][$i]["MIN_PRICE"] = $arPrices[$prid];
					}	
					else
						$arResult["SEARCH"][$i]["MIN_PRICE"] = $arPrices[$prid];
				}
				...
				if(isset($arResult["SEARCH"][$i]["MIN_PRICE"])){
					$arResult["SEARCH"][$i]["PRICES"] = array();
					$arResult["SEARCH"][$i]["PRICES"]["PRICE"] = $arResult["SEARCH"][$i]["MIN_PRICE"];
				}
			...
			{	 	 
				unset($arResult["SEARCH"][$i]["MIN_PRICE"]);
				unset($arResult["SEARCH"][$i]["PRICES"]);
				$properPrice =  CIBlockElement::GetProperty($arItem["PARAM2"], $arItem["ITEM_ID"], array("sort" => "asc"), Array("CODE"=>$arParams["PRICE_CODE"]));
				if($priceElement = $properPrice->Fetch())
				{
					if(isset($priceElement)){
						$arResult["SEARCH"][$i]["PRICES"] = array();
						$arResult["SEARCH"][$i]["PRICES"]["PRICE"] = $priceElement["VALUE"];
					} 
				}
			}
		/* price end */
				...
				if(CModule::IncludeModule("iblock"))
				{
					...
					else
					{
						...
						if(CModule::IncludeModule('yenisite.resizer2'))
						{
							...
							$pathResizeImage = CResizer2Resize::ResizeGD2($pathImage, $arParams['PHOTO_SIZE']);
						}
						else
						{
