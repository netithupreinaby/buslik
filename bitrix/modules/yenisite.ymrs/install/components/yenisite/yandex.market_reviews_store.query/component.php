...
if ($this->StartResultCache(false, $USER->GetGroups()))
{
	...
	$query_page = 'https://api.content.market.yandex.ru/v1/shop/'.$arParams["SHOPID"].'/opinion.json?geo_id=0&page='.$arParams["PAGE"].$strOptions;
	...
	if ($json['errno'] == '0'):
		...
		if ($arReviews["shopOpinions"]):
			if(LANG_CHARSET != 'UTF-8') { //'UTF-8' to 'windows-1251' or other
				foreach ($arReviews["shopOpinions"]["opinion"] as &$review) {
			...
			if(isset($count_one))
			{
				$arReviews["shopOpinions"]["count"] = 1;
				$arReviews["shopOpinions"]["page"] =  IntVal($_REQUEST["PAGE"]) ;
				$arReviews["shopOpinions"]["opinion"] = array($arReviews["shopOpinions"]["opinion"][$count_one]);
			}
			/*---/-API FIX*/
		endif;
