...
class CYenisiteInfoblockpropsplus
{
        ...
        switch ($arParams['action']) {
            case 'add_group':
                ...
                if (intval($arParams['group_id']) == 0 && intval($arParams['iblock_id']) > 0 && strlen($arParams['group_name']) > 0) {
                    $arParams['group_id'] = self::AddGroup($arParams['iblock_id'], $arParams['group_name'], $arParams['group_sorting'], $arParams['group_section'], $arParams['in_section_edit']);
                    ...
                    self::UpdateGroup($arParams['group_id'], $arParams['group_name'], $arParams['group_sorting'], $arParams['iblock_id'], $arParams['group_section'], $arParams['in_section_edit']);
                }
    ...
    static function UpdateGroup($group_id, $group_name, $group_sorting, $iblock_id, $section_id, $in_section_edit = 'n')
    {
        global $DB;
        $iblock_id = intval($iblock_id);
        ...
        if ($DB->Query('update ' . self::tblGroups . ' SET `NAME` = "' . $DB->ForSql(self::ConvertText($group_name)) . '"' . $sortUpdate . $update_section_id . ' WHERE `ID` = ' . $group_id, true) == false) {
            return self::AddGroup($iblock_id, $group_name, $group_sorting, $section_id);

        }
    }

    static function AddGroup($iblock_id, $group_name, $group_sorting, $section_id = 0, $in_section_edit = 'n')
    {
        global $DB;
        ...
        if(empty($res['ID'])) {
            if ($DB->Query('insert into ' . self::tblGroups . ' ( `NAME`, `IBLOCK_ID`, `SORT`, `SECTION_ID`) values ( "' . $DB->ForSql(self::ConvertText($group_name)) . '",' . $iblock_id . ', ' . $group_sorting . ', ' . $section_id . ' )', true) == false) {
                $res = $DB->Query('select ID from ' . self::tblGroups . ' where name = "' . $DB->ForSql(self::ConvertText($group_name)) . '" and iblock_id = ' . $iblock_id)->Fetch();
                return $res['ID'];
            } else {
                return $DB->LastID();
