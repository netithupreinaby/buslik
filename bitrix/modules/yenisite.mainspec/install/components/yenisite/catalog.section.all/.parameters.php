...
else
{
	if($arCurrentValues["IBLOCK_TYPE"]){
			$val = str_replace("#SITE_ID#", SITE_ID, $arCurrentValues["IBLOCK_TYPE"]);
			$val = preg_replace('\'={}#."', '', $val);
...
if(!is_array($arCurrentValues["IBLOCK_ID"]) && $arCurrentValues["IBLOCK_ID"] > 0)
{
	...
	if($OFFERS_IBLOCK_ID)
	{
		...
		print_r($arCurrentValues["IBLOCK_TYPE"]);
		echo "</pre>";
}
...
if (CModule::IncludeModule('yenisite.geoipstore')) {
	...
	$arComponentParameters['PARAMETERS']['STORE_CODE'] = array(
		"PARENT" => "ADDITIONAL_SETTINGS",
