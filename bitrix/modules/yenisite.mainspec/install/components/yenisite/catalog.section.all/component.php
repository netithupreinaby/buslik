...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($arParams["IBLOCK_ID"] > 0)
		...
		if(count($ibs))
			$arFilter['IBLOCK_ID'] = $ibs;
	$arResult["FILTER_IBLOCK_ID"] = $arFilter['IBLOCK_ID'];
	$arResult["ITEMS"] = array();	
	$arResult["COUNT"] = 0;

	if($arParams["IS_YS_MS"] && $_REQUEST["ys_ms_sef"] !== "y")
	{
		$arNavParams = array(
			'iNumPage' => 1,
			'nPageSize' => 10,
		...
		$arResult["COUNT"] += CIBlockElement::GetList(array(), array_merge($arrFilter, $arFilter), array(), false);	
	}
	
	if($_REQUEST["ys_ms_ajax_call"] === "y")
		if($_REQUEST["iNumPage"])
			$arNavParams['iNumPage'] = IntVal($_REQUEST["iNumPage"]);
	...
	while($obElement = $rsElements->GetNextElement())
	{
		...
		if(!in_array($arItem["IBLOCK_ID"], $iblockIDS))
			$iblockIDS[] = $arItem["IBLOCK_ID"];
...
foreach($iblockIDS as $ibid):
	...
	$arOffers = CIBlockPriceTools::GetOffersArray(
		...
		if(!empty($arOffers))
		{
			...
			{
				if($arResult["ITEMS"][$i]["IBLOCK_ID"] == $ibid){
