...
if (COption::GetOptionString("sebekon.deliveryprice", "DP_DISABLE_JQUERY_ADMIN", "N")=='N') {
	$jquery = '
				<script type="text/javascript" src="/bitrix/js/sebekon.deliveryprice/jquery-1.8.0.min.js"></script>
				<script type="text/javascript">
					var $sebekon_jq_delivery = jQuery.noConflict();  
				</script>		
			';
	$APPLICATION->AddHeadString($jquery);
	$APPLICATION->AddHeadScript("/bitrix/js/sebekon.deliveryprice/jquery-1.4.4.min.js");
}

$APPLICATION->AddHeadScript("/bitrix/js/sebekon.deliveryprice/deliveryprice.js");
if(count($PROP)>0):
	...
	foreach($PROP as $prop_code=>$prop_fields):
		...
		<tr id="tr_PROPERTY_<?echo $prop_fields["ID"];?>">
			...
			<td>
				...
				<?if($prop_fields['CODE'] != "ZONE_COORDS"):?>
				...
				<?else:
					...
					foreach($prop_values as $key => $value):
						$color = current($PROP["ZONE_COLOR"]["VALUE"]);
						?>
						<script src="//api-maps.yandex.ru/2.0/?load=package.full,util.json&lang=ru-RU&wizard=sebekon.deliverycalc" type="text/javascript"></script>
						...
						<script type="text/javascript">
							...
							function init () {
								...
								/*<?print_r($PROP["ZONE_COLOR"]["VALUE"])?>*/
								
								var $mapElement = $sebekon_jq_delivery('#map');
								...
								var coordinates = getPointsFromCoords(c);		
								...
								if (maxx!=0 && minx!=0 && maxy!=0 && miny!=0) {
									...
									myMap.setBounds([[minx,miny],[maxx,maxy]], {checkZoomRange: true,  precizeZoom: true, callback:function(){				
										if (myMap.getZoom()<8 && diff<81) myMap.setZoom(8);}
									});
								}
								...
								myGeoobject.events.add('geometrychange', function (event) {
									saveGeometry(myGeoobject.geometry.getCoordinates());
								});
							...
							function stringify (coords) {
								var res = '';
								if ($sebekon_jq_delivery.isArray(coords)) {
									res = '[ ';
							...
							function saveGeometry (coords) {
								$sebekon_jq_delivery('#PROP_<?=$prop_fields["ID"]."_".$key?>').val(stringify(coords));
							}
						</script>
					
						<div id="map" style="width: 600px; height: 400px"></div>
						
						<input type="hidden" name="PROP[<?=$prop_fields["ID"]?>][<?=($key ? $key : "n".$start++)?>]" id="PROP_<?=$prop_fields["ID"]."_".$key?>" value="<?=$value?>"> 
						<?break;//must be only one
					endforeach;?>
		...
		<?
		if($prop_fields['CODE'] == "ZONE_COLOR"):
			?>
			<script type="text/javascript" src="/bitrix/js/sebekon.deliveryprice/farbtastic/farbtastic.js"></script>
			<link rel="stylesheet" media="screen" href="/bitrix/js/sebekon.deliveryprice/farbtastic/farbtastic.css" type="text/css" />
				
			<script language="JavaScript" >
				$sebekon_jq_delivery(function(){
					var input = $sebekon_jq_delivery('#tr_PROPERTY_<?=$prop_fields["ID"];?>').find('input:first');
					if(input.val()==''){
						input.val('#ff0000');
					}
					$sebekon_jq_delivery('<div id="farbtastic<?=$prop_fields["ID"]?>"></div>').insertAfter(input);
					
					$sebekon_jq_delivery('#farbtastic<?=$prop_fields["ID"]?>').farbtastic(input);
				});
			</script>
		<?endif;
...
if ($bOffers)
{
	...
	$strSubElementAjaxPath = '/bitrix/admin/iblock_subelement_admin.php?WF=Y&IBLOCK_ID='.$intSubIBlockID.'&type='.$strSubIBlockType.'&lang='.LANGUAGE_ID.'&find_section_section=0&find_el_property_'.$arSubCatalog['SKU_PROPERTY_ID'].'='.((0 == $ID) || ($bCopy) ? '-'.$TMP_ID : $ID).'&TMP_ID='.urlencode($strSubTMP_ID);
...
)
{
	...
	echo
		...
		' <a href="/bitrix/admin/iblock_edit.php?type='.htmlspecialcharsEx($type).'&amp;lang='.LANG.'&amp;ID='.$IBLOCK_ID.'&amp;admin=Y&amp;return_url='.urlencode("/bitrix/admin/iblock_element_edit.php?ID=".$ID.($WF=="Y"?"&WF=Y":"")."&lang=".LANG. "&type=".htmlspecialcharsEx($type)."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section).(strlen($return_url)>0?"&return_url=".UrlEncode($return_url):"")).'">',
	...
	//////////////////////////
} elseif (CModule::IncludeModuleEx('sebekon.deliveryprice') == MODULE_DEMO_EXPIRED) {
    echo GetMessage('sebekon_DP_module_demo_expired');
} else {
	echo GetMessage('sebekon_DP_please_install_module');
}
?>