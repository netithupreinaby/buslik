...
if (!function_exists('yenisite_GetPropPicSrc')) {
	function yenisite_GetPropPicSrc($arElement, $prop_code)
	{
		...
		if($arPropFile)
		{
			...
			{
				$arPropFile = array_diff($arPropFile, array(''));
				if(intval(end($arPropFile)) > intval($arPropFile[0]))
				{
					return $arPropFile[0];  //$arPropFile[0] - is bad because sometimes bitrix return array of values in back order (ID=>DESC)
				}
				else	
				{
					return min($arPropFile);
				}
			}
...
if (!function_exists('yenisite_GetPicSrc')) {
	function yenisite_GetPicSrc ($arElement, $arParamsImage = 'DETAIL_PICTURE', $default_image_code = 'MORE_PHOTO', $bProductCheck = true)
	{
		...
		if(!empty($arElement['PRODUCT_ID'])) {
			...
			unset($arElement['PRODUCT_ID']);
		}
		if(empty($arElement['IBLOCK_ID']))
		{
			if ($res = CIBlockElement::GetByID($arElement['ID'])->Fetch()) {
				$arElement = array_merge($arElement, $res);
			}
		}
		...
		if(!$picsrc)
		{
			...
			if (!$picsrc && CModule::IncludeModule('catalog') && $bProductCheck) {
				$productInfo = CCatalogSku::GetProductInfo($arElement['ID'], $arElement['IBLOCK_ID']);
				if (is_array($productInfo)) {
					$arProduct = CIBlockElement::GetByID($productInfo['ID'])->GetNext();
					$picsrc = yenisite_GetPicSrc($arProduct, $arParamsImage, $default_image_code, false);
				}
			}
		}
		return $picsrc ;
