...
<?elseif($arResult["ALLOW_ANONYMOUS"]=="N" || $_REQUEST["authorize"]=="YES" || $_REQUEST["register"]=="YES"):?>
	...
	<tr valign="top">
		<td width="40%">
			...
			<input class="txt" type="text" name="LOGIN" value="<?echo $arResult["REQUEST"]["LOGIN"]?>" size="20" /></p>
			<p><?echo GetMessage("adm_auth_pass")?><span class="starrequired">*</span><br />
			<input class="txt" type="password" name="PASSWORD" size="20" value="<?echo $arResult["REQUEST"]["PASSWORD"]?>" /></p>
		</td>
		<td width="60%">
	...
	<tfoot><tr><td colspan="2"><input class="button" type="submit" name="Save" value="<?echo GetMessage("adm_auth_butt")?>" /></td></tr></tfoot>
	</table>
	...
	<?if($arResult["ALLOW_REGISTER"]=="Y"):
		...
		<tr valign="top">
			...
			<input class="txt" type="text" name="NEW_LOGIN" value="<?echo $arResult["REQUEST"]["NEW_LOGIN"]?>" size="20" /></p>
			<p><?echo GetMessage("adm_reg_pass")?><span class="starrequired">*</span><br />
			<input class="txt" type="password" name="NEW_PASSWORD" size="20" value="<?echo $arResult["REQUEST"]["NEW_PASSWORD"]?>" /></p>
			<p><?echo GetMessage("adm_reg_pass_conf")?><span class="starrequired">*</span><br />
			<input class="txt" type="password" name="CONFIRM_PASSWORD" size="20" value="<?echo $arResult["REQUEST"]["CONFIRM_PASSWORD"]?>" /></p>
			<p><?echo GetMessage("subscr_email")?><span class="starrequired">*</span><br />
			<input class="txt" type="text" name="EMAIL" value="<?=$arResult["SUBSCRIPTION"]["EMAIL"]!=""?$arResult["SUBSCRIPTION"]["EMAIL"]:$arResult["REQUEST"]["EMAIL"];?>" size="30" maxlength="255" /></p>
		<?
			/* CAPTCHA */
			...
			?>
				...
				<input class="txt" type="text" name="captcha_word" size="30" maxlength="50" value="" /></p>
			<?endif;?>
			</td>
			<td width="60%">
				<?if($arResult["ALLOW_ANONYMOUS"]=="Y"):?>
		...
		<tfoot><tr><td colspan="2"><input class="button" type="submit" name="Save" value="<?echo GetMessage("adm_reg_butt")?>" /></td></tr></tfoot>
		</table>
		<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
			<input type="hidden" name="RUB_ID[]" value="<?=$itemValue["ID"]?>">
		<?endforeach;?>
