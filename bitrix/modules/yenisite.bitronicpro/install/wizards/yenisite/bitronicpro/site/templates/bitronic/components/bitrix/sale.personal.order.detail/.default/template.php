...
<?if(strlen($arResult["ERROR_MESSAGE"])<=0):?>
	...
	<tr>
		...
		<td width="60%"><?
				echo "<b>".str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arResult["PRICE_FORMATED"])."</b>";
	...
	<tr>
		...
		<td width="60%"><?
			...
			{
				?>&nbsp;<a class="button" href="<?=$arResult["URL_TO_CANCEL"]?>"><?=GetMessage("SALE_CANCEL_ORDER")?></a><?
			}
			?></td>
	</tr>
	<tr>
		...
		<tr>
			<td colspan="2" align="center">
				...
				if ($arResult["PAY_SYSTEM"]["PSA_NEW_WINDOW"] == "Y")
				{
					...
					<a class="button" href="<?=$arResult["PAY_SYSTEM"]["PSA_ACTION_FILE"]?>" target="_blank"><?=GetMessage("SALE_REPEAT_PAY")?></a>
					<?
				}
				else
				{
	...
	<tr>
		<td align="right" width="40%"><?=GetMessage('SPOD_ORDER_TRACKING_NUMBER')?>:</td>
			...
			<table class="sale_personal_order_detail data-table">
				...
				foreach($arResult["BASKET"] as $val)
				{
					...
					<tr>
						...
						<td><?=$val["QUANTITY"]?></td>
						<td align="right"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $val["PRICE_FORMATED"])?></td>
					</tr>
					<?
				}
				?>
				...
				<tr>
					...
					<?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arResult["TAX_VALUE_FORMATED"])?>					
					</td>
				</tr>
				<?endif;?>
				...
				<tr>
					...
					<td align="right" colspan="5">
					<?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arResult["PRICE_DELIVERY_FORMATED"])?>
					</td>
				</tr>
				<?endif;?>
				<tr>
					...
					<td align="right" colspan="5">
					<?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arResult["PRICE_FORMATED"])?>
					</td>
				</tr>
			</table>
		</td>
