...
//==================== edost START
		if (CModule::IncludeModule('edost.locations')) {
			...
			if ($code == 'CITY' || $code == 'LOCATION' || $code == 'ZIP') {
				$cityNowID = 0;
				$cityActive = 'Y';
				if ($code == 'LOCATION') {
					foreach ($arProperties["VARIANTS"] as $arVariant)
						if ($arVariant["SELECTED"] == "Y") {
							if ($arVariant["set_location"] == "N") $set_location = 'N';

							$cityNowID = $arVariant["ID"];
							break;
						}

					$cityActive = '';
					foreach ($arSource as $arProperties2)
						if ($arProperties2["CODE"] == 'CITY') {
							$cityActive = 'Y';
							break;
						}
				}
				...
				$GLOBALS["APPLICATION"]->IncludeComponent('edost:locations', '', array(
					"QUICK_CITY" => "Y", // "Y" - включен список городов для быстрого выбора
//					"QUICK_CITY_DATA" => "Москва, Екатеринбург, Санкт-Петербург, Новосибирск,, Пермь,, Красноярск,, Нижний Новгород", // Список городов для быстрого выбора (города указывать через запятую)

					"ZIP_HELP" => 'Y', // "Y" - показывать у индекса ссылку на страницу с помощью
					"SET_RUSSIA" => 'Y', // "Y" - устанавливать по умолчанию Россию
					"SET_LOCATION" => (isset($set_location) ? $set_location : 'Y'),
					"IN_TABLE" => 'Y', // "Y" - вывод через таблицу (тэги <tr> и <td>)

//					"ZIP_ONLYDIGIT" => "Должны быть только цифры!",
//					"ZIP_WARNING1" => "Такого индекса НЕ существует!",
//					"ZIP_WARNING2" => "Индекс НЕ соответствует региону!",
//					"LOADING_IMAGE" => 'loading_small_f2.gif', // картинка расчета доставки и проверки индекса - в папке bitrix/components/edost/locations/images/
					"STYLE" => ($code == 'LOCATION' ? '#edost_LOCATION_field td { border: 0px !important; padding: 2px !important; }' : ''), // выводится в тэгах <style> стили </style>
					"MAIN_TAG" => '<tr'.($code == 'CITY' ? ' id="citynew" style="display: none;"' : '').'>',
					"MAIN_TAG_END" => '</tr>',
					"NAME_TAG" => '<td class="name">',
					"NAME_TAG_END" => '</td>',
					"FIELD_TAG" => '<td id="edost_'.$code.'_field">', //.($code == 'LOCATION' ? ' style="padding-left: 10px;"' : '').'>',
					"FIELD_TAG_END" => '</td>', // ($code == 'ZIP' && strlen($arProperties["DESCRIPTION"]) > 0 ? '<br><small>'.$arProperties["DESCRIPTION"].'</small>'.'<script language="javascript"> edost_SetHTML("zip_info", ""); </script>' : '').'</td>',

					"CITY_ACTIVE" => $cityActive, // "Y" - в свойствах заказа есть поле 'CITY'
					"CITY_NOW_ID" => $cityNowID, // код выбранного местоположения
					"CODE" => $code,
					"VALUE" => $arProperties["VALUE"],
					"REQUIED_FORMATED" => 'NO_CHANGE', //$arProperties["REQUIED_FORMATED"], // "NO_CHANGE" - выводить название поля (параметр "NAME") без изменений
					"NAME" => $arProperties["NAME"].'<span class="sof-req">*</span>',
			...
			<tr>
				...
				<td>
					...
					if($arProperties["TYPE"] == "CHECKBOX")
					{
						...
						<input type="checkbox" class="checkbox" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" value="Y"<?if ($arProperties["CHECKED"]=="Y") echo " checked";?>>
						<?
					}
					elseif($arProperties["TYPE"] == "TEXT")
					{
						?>
						<input type="text" class='txt' maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>">
						<?
					}
