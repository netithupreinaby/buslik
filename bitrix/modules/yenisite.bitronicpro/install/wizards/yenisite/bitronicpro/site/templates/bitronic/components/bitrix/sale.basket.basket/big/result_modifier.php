...
if ($arParams["USE_SLIDER_MOUSEWHEEL"] != 'N' && CModule::IncludeModule('yenisite.mainspec')) {
...
$arGroupItem = array ('AnDelCanBuy', 'DelDelCanBuy', 'nAnCanBuy');
foreach($arGroupItem as $item_group)
{
	foreach($arResult['ITEMS'][$item_group] as &$arItem)
	{
		$arProduct = false;
		//if($arItem['CAN_BUY'] == 'Y')
		//{
			if(CModule::IncludeModule("iblock")/*  && $arParams['IMAGE'] */)
			{
				...
				else
				{
					...
					if($arElement = $dbElement->GetNext())
					{
						...
						if(!$image)
							$image = CFile::GetPath(yenisite_GetPicSrc($arElement));
					}
...
unset($obCache);
	$arJSParams = array(
		"RESIZER_SET" => $arParams['BASKET_PHOTO']
	);
?>
<script>
	var BasketParams = <? echo CUtil::PhpToJSObject($arJSParams, false, true); ?>;
</script>