				...
				<td valign="top">
					<?switch($code):
						case "NAME":							
								...
								if(!$path) $path = CFile::GetPath(yenisite_GetPicSrc($res1));//$path = CFile::GetPath($res1[DETAIL_PICTURE]);		
								global $basket_set;
					?>		
					<div>
							<span class="stick_img">
								...
								<img src="<?=CResizer2Resize::ResizeGD2($path, $arParams["COMPARE_IMG"]);?>" alt="" />
							</span>
							</div>
							<br/>
							<a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?=$arElement[$code]?></a><?
							if($arElement["CAN_BUY"] || $arElement['CATALOG_AVAILABLE']):
								?><div class="ys-compare-basket-button" ><a class="button2" id="b-<?=$arElement['ID'];?>" href="<?=$arElement["ADD_URL"]?>" rel="nofollow"><span><?=GetMessage("CATALOG_COMPARE_BUY"); ?></span></a></div><?
