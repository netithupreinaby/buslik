...
function yenisite_set_quantity(id, sign, url, bxsessid, titles)
{
	...
	if(Number(current_quantity) >= 0){
		...
		key = $('#YS_BS_KEY_'+id).val();

		id_basket_element = id_basket_element.replace('YS_BS_QUANTITY_', '');
		$.post(url, {bxsessid: bxsessid, id_basket_element: id_basket_element, new_quantity: new_quantity, action: 'setQuantity', key: key},
		function(data) {		
