...
if (($REQUEST_METHOD == "POST" || $CUR_FILE_POS > 0) && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 3 /*&& isset($_REQUEST["import_now"])*/)
	{
	
		if($_REQUEST['save_btn'])
		{
			...
			global $DB;
			$strSql = "UPDATE yen_market_import_profile SET
				file_fields='".$fields."',
				PATH2IMAGE_FILES='".htmlspecialchars($_REQUEST["PATH2IMAGE_FILES"])."',
				PATH2PROP_FILES='".htmlspecialchars($_REQUEST["PATH2PROP_FILES"])."',
				IMAGE_RESIZE='".htmlspecialchars($_REQUEST["IMAGE_RESIZE"])."',
				max_execution_time='".htmlspecialchars($_REQUEST["max_execution_time"])."',
				inFileAction='".htmlspecialchars($_REQUEST["inFileAction"])."',
				outFileAction='".htmlspecialchars($_REQUEST["outFileAction"])."' WHERE id=".$profile_id."
			";
			if($fields)
				$res = $DB->Query($strSql, false, $err_mess.__LINE__);
			LocalRedirect('/bitrix/admin/yci_market_import_list.php?lang=ru');
			die();
...
function FetchAssoc(&$csvFile)
{
	...
	return $ar;
		...
		/*echo "<div id='position'>{$csvFile->GetPos()}</div>";
		print_r($_REQUEST);
		*/
...
if($_REQUEST["start"]=="Y" && $_REQUEST["STEP"] < 4)
{

    echo "<b>".GetMessage("LOADING")."</b>";
...
if (!$bAllLinesLoaded)
{
	...
	function DoNext()
	{
		//document.getElementById('position').innerHTML='fgfgfdg';		
		window.location="<?echo $APPLICATION->GetCurPage()?>?lang=<?echo LANG?>&<?echo $strParams ?>?start=Y&profile_id=<?=htmlspecialcharsbx(intval($_REQUEST['profile_id']))?>&profile_name=<?=htmlspecialcharsbx($_REQUEST['profile_name'])?>";
	}
...
if ($STEP == 1)
{
    $PROFILE_NAME = htmlspecialchars($_REQUEST["profile_name"]);
    ?>

    <? if(!isset($_REQUEST["profile_id"])): ?>
        <input type="hidden" name="profile_id" value="new">
    <?else:?>
        <? $profile_id = htmlspecialchars($_REQUEST["profile_id"]);?>


    <?
        global $DB;
        $strSql = "SELECT * FROM yen_market_import_profile WHERE id=".$profile_id;
        $res = $DB->Query($strSql, false, $err_mess.__LINE__);
        $prof = $res->GetNext();        
        $URL_DATA_FILE = $prof["file"];
        $PROFILE_NAME = $prof["name"];
        $IBLOCK_ID = $prof["iblock_id"];
    ?>


        <input type="hidden" name="profile_id" value="<?=htmlspecialchars($_REQUEST["profile_id"])?>">
    <?endif?>
        <tr>
		<td><?=GetMessage("PROFILE_NAME")?>:</td>
		<td>
			<input type="text" name="profile_name" value="<?=$PROFILE_NAME?>" size="30">
                </td>
        </tr>
...
if ($STEP == 2)
{
    ...
    if($profile_id == 'new')
    {
        ...
        global $DB;
        $strSql = "INSERT INTO yen_market_import_profile(name, file, iblock_type, iblock_id) VALUES(
            '".htmlspecialchars($_REQUEST["profile_name"])."',
            '".htmlspecialchars($_REQUEST["URL_DATA_FILE"])."',
            '".htmlspecialchars($_REQUEST["IBLOCK_TYPE_ID"])."',
            ".htmlspecialchars($_REQUEST["IBLOCK_ID"])."
        )";
        $res = $DB->Query($strSql, false, $err_mess.__LINE__);
    ...
    else
    {
        ...
        global $DB;
        $strSql = "UPDATE yen_market_import_profile SET 
            name='".htmlspecialchars($_REQUEST["profile_name"])."', 
            file='".htmlspecialchars($_REQUEST["URL_DATA_FILE"])."', 
            iblock_type='".htmlspecialchars($_REQUEST["IBLOCK_TYPE_ID"])."',
            iblock_id=".htmlspecialchars($_REQUEST["IBLOCK_ID"])." WHERE id=".$profile_id;
            $res = $DB->Query($strSql, false, $err_mess.__LINE__);
    ...
    <?
        ...
        $PROFILE_NAME = $prof["name"];
        $IBLOCK_ID = $prof["iblock_id"];
        $fields_type = $prof["fields_type"];
        $delimiter_r = $prof["delimiter"];
        $delimiter_other_r = $prof["delimiter_other"];
        $first_names_r = $prof["first_names"];
        $first_names_f = $prof["first_names2"];
        $metki_f = $prof["metki"];
    ?>
...
if ($STEP == 3)
{
        ...
        global $DB;
        $strSql = "UPDATE yen_market_import_profile SET
            delimiter='".htmlspecialchars($_REQUEST["delimiter_r"])."',
            first_names='".htmlspecialchars($_REQUEST["first_names_r"])."',
            metki='".htmlspecialchars($_REQUEST["metki_f"])."',
            data='".htmlspecialchars($_REQUEST["data"])."',
            first_names2='".htmlspecialchars($_REQUEST["first_names_f"])."',
            fields_type='".htmlspecialchars($_REQUEST["fields_type"])."' WHERE id=".$profile_id."
        ";
        $res = $DB->Query($strSql, false, $err_mess.__LINE__);
	...
	for ($k = 0; $k < $NUM_CATALOG_LEVELS; $k++)
	{
        ...
        $first_names_r = $prof["first_names"];
        $first_names_f = $prof["first_names2"];
		$max_execution_time = $prof["max_execution_time"]>0?$prof["max_execution_time"]:5;
        $metki_f = $prof["metki"];
        $fields = $prof["file_fields"];
		$IMAGE_RESIZE = $prof['IMAGE_RESIZE'];
		$PATH2IMAGE_FILES = $prof['PATH2IMAGE_FILES'];
		$PATH2PROP_FILES = $prof['PATH2PROP_FILES'];
		$outFileAction = $prof['outFileAction'];
		$inFileAction = $prof['inFileAction'];
		...
		if(!empty($fields))
        {
            ...
            {
                $field = explode("###", $field);
                if($field[1]?$field[1]:$arDataFileFields[$i])
                {
                    $arFields[] = $field[1]?$field[1]:$arDataFileFields[$i];
                    $i++;
                }

            }			

            foreach($arDataFileFields as $k=>$f)
            {
                $arDataFileFields[$k] = $arFields[$k];
            }
            
            //if(count($arDataFileFields) == count($arFields))
                //$arDataFileFields = $arFields;
        }
			...
			<td valign="top" width="60%">
				<select name="field_<?echo $i ?>">
					...
					for ($j = 0; $j < count($arAvailFields); $j++)
					{
						...
						?>
						<?$optionName = (htmlspecialchars($arAvailFields[$j]["name"]) != '' ? htmlspecialchars($arAvailFields[$j]["name"]) : htmlspecialchars($arAvailFields[$j]["name"],ENT_HTML5, 'cp1251'));
						$optionName = (empty($optionNmae) ? $arAvailFields[$j]["name"] : $optionNmae)?>
						<option value="<?=$arAvailFields[$j]["value"] ?>" <?if ($bSelected) echo "selected" ?>><?= $optionName?></option>
						<?
					}
					?>
				</select>
...
?>
				...
				if (!$bAllLinesLoaded){
					...
					?>
					&nbsp;&nbsp;	<img style='' src='http://<?=$_SERVER['SERVER_NAME']?>/bitrix/modules/yenisite.market/admin/ajax-loader.gif' /> 
					<?	
					}
				else
        ...
        <?//if(isset($_REQUEST["import_now"])):?>
	<tr>
		<td valign="middle" colspan="2" nowrap>
		<?
				$pos = $csvFile->GetPos(); 
				$all = $csvFile->iFileLength;
				$per = $pos/$all*100;
		?>
			...
			?>
			<div style='width: 100%; height: 10px; border: 1px solid #888888; background: #ffffff; margin-top: 10px;'>
				<div style='width: <?=$per?>%; height: 10px; background: #54AAFF;'></div>
			</div>
		</td>
	</tr>
        <?//endif?>
...
<?if ($STEP < 4):?>
	...
	<?if($STEP==3):?>
		<input type="submit" value="<?echo GetMessage("SAVE_BTN_ALT")?>" name="save_btn">
		<input type="hidden" value="<?echo ($STEP==3)?GetMessage("IBLOCK_ADM_IMP_NEXT_STEP_F"):GetMessage("IBLOCK_ADM_IMP_NEXT_STEP") ?> &gt;&gt;" name="submit_btn">
	<?else:?>
...
<?else:?>
	<!--<input type="submit" name="backButton2" value="&lt;&lt; <?echo GetMessage("IBLOCK_ADM_IMP_2_1_STEP") ?>">-->
        <a href="/bitrix/admin/yci_market_import_list.php"><?echo GetMessage("IBLOCK_ADM_IMP_2_1_STEP") ?></a>
<?endif;?>
