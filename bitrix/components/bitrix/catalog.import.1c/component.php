...
{
	echo "failure\n",GetMessage("CC_BSC1_ERROR_SOURCE_CHECK"), "\nDebug: ". print_r($_SESSION['fixed_session_id'], true);
}
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	{                            
        ...
        $bufferSize = ceil($total/1000);
        $_SESSION['iteratorId']=0;
		if (($total > 0) && is_resource($fp))
		{
			$obXMLFile = new CIBlockXMLFile;
			
            
			if ($obXMLFile->ReadXMLToDatabase($fp, $NS, $bufferSize))
			{
				$NS["STEP"] = 3;
			...
			{            
                $asdf = $obXMLFile->GetFilePosition();
