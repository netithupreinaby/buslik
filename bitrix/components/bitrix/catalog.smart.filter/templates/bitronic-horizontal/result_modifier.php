...
$arParams['VISIBLE_ROWS_COUNT'] = isset($arParams['VISIBLE_ROWS_COUNT']) ? intval($arParams['VISIBLE_ROWS_COUNT']) : 2;
$visibleItems = $arParams['VISIBLE_ROWS_COUNT'] * 3;
...
foreach ($arResult['ITEMS'] as &$arItem) {
	...
	if(!empty($arItem["VALUES"]) && $arItem["PROPERTY_TYPE"] != "N") {
		...
		foreach ($arItem["VALUES"] as $key => $value) {
			...
			if ($itemNum > $visibleItems) $arParams['START_EXPANDED'] = 'Y';
			if ($i <= 5) continue;
			$arItem['EXPAND'] = true;
			break;
		}
