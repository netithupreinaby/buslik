...
function yenisite_set_quantity(id, sign, url, titles, control_quantity, product_id, quantity_logic, err3, allow_float_q)
{
	...
	if(Number(current_quantity) >= 0){
		...
		$.post(url, {id_basket_element: id_basket_element, new_quantity: new_quantity, action: 'setQuantity', control_quantity: control_quantity, product_id:product_id, allow_float_q:allow_float_q},
		function(data) {
			data = $.trim(data) ;
