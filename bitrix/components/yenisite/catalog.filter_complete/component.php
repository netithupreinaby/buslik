...
$arResult["FORM_ACTION"] = isset($_SERVER['REQUEST_URI'])? htmlspecialcharsEx($_SERVER['REQUEST_URI']): "";
...
foreach($arParams["FIELD_CODE"] as $field_code)
{
	...
	switch ($field_code)
	{
		...
		case "SEARCHABLE_CONTENT":
			$field_res = '<input type="text" name="'.$name.'" size="'.$arParams["TEXT_WIDTH"].'" value="'.htmlspecialcharsEx($value).'" />';
		...
		case "SHOW_COUNTER":
			...
			$field_res = GetMessage("CC_BCF_OT").'&nbsp;<input type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.htmlspecialcharsEx($value).'" />';
			...
			$field_res .= '&nbsp;'.GetMessage("CC_BCF_DO").'&nbsp;<input type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.htmlspecialcharsEx($value).'" />';
	...
	if($field_res)
		$arResult["ITEMS"][] = array(
			"NAME" => htmlspecialcharsEx(GetMessage("IBLOCK_FIELD_".$field_code)),
			"INPUT" => $field_res,
...
foreach($arResult["arrProp"] as $prop_id => $arProp)
{
	...
	switch ($arProp["PROPERTY_TYPE"])
	{
		case "L":
			...
			if($arProp["LIST_TYPE"] == "L")
			{
				...
				foreach($arProp["VALUE_LIST"] as $key=>$val)
				{
					...
					$res .= ' value="'.htmlspecialcharsEx($key).'">'.htmlspecialcharsEx($val).'</option>';
				}
				$res .= '</select>';
		...
		case "N":
			...
			$res .= '&nbsp;'.GetMessage("CC_BCF_DO").'&nbsp;<input id="'.$arProp["CODE"].'-max" type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.htmlspecialcharsEx($value).'" />';
			...
			if ( count($allValues['PROPERTY_'.strtoupper($arProp["CODE"]).'_VALUE']) > 0 )
			{
				...
				foreach($allValues['PROPERTY_'.strtoupper($arProp["CODE"]).'_VALUE'] as $v)
				{
					...
					if($v == $value || in_array($v, $value))
						...
						$checked = "";
					$res .= '<option '.$checked.' value="'.htmlspecialcharsEx($v).'">'.$v.'</option>';
				}			
				$res .= '</select>';
				${$FILTER_NAME}["PROPERTY"][$arProp["CODE"]] = $value;
		...
		case "G":
			...
			$name = $FILTER_NAME."_pf[".$arProp["CODE"]."]";
			$value = $arrPFV[$arProp["CODE"]];
			$res .= '<input type="text" name="'.$name.'" size="'.$arParams["TEXT_WIDTH"].'" value="'.htmlspecialcharsEx($value).'" />';
...
*/	
		...
		case "N":
			...
			$res .= GetMessage("CC_BCF_OT").'&nbsp;<input type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.htmlspecialcharsEx($value).
				'" />';
			...
			$res .= '&nbsp;'.GetMessage("CC_BCF_DO").'&nbsp;<input type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.htmlspecialcharsEx($value).'" />';
...
foreach($arResult["arrPrice"] as $price_code => $arPrice)
{
	...
	$res_price .= GetMessage("CC_BCF_OT").'&nbsp;<input id="CATALOG_PRICE_'.$arPrice['ID'].'-min" type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.
		htmlspecialcharsEx(round($value)).'" />';
	...
	$res_price .= '&nbsp;'.GetMessage("CC_BCF_DO").'&nbsp;<input  id="CATALOG_PRICE_'.$arPrice['ID'].'-max" type="text" name="'.$name.'" size="'.$arParams["NUMBER_WIDTH"].'" value="'.htmlspecialcharsEx(round($value)).'" />';

	$arResult["ITEMS"][] = array(
		"NAME" => htmlspecialcharsEx($arPrice["TITLE"]), 
		"INPUT" => $res_price,
...
foreach(array_merge($_GET, $_POST) as $key=>$value)
{
	...
	{	
		$arResult["ITEMS"][] = array(
			"HIDDEN" => true,
			"INPUT" => '<input type="hidden" name="'.htmlspecialcharsEx($key).'" value="'.htmlspecialcharsEx($value).'" />',
		);
	}
}
