<?
$MESS ['IBLOCK_DETAIL_URL'] = "URL, ведущий на страницу с содержимым элемента раздела";
$MESS ['T_IBLOCK_DESC_ASC'] = "По возрастанию";
$MESS ['T_IBLOCK_DESC_DESC'] = "По убыванию";
$MESS ['T_IBLOCK_DESC_FID'] = "ID";
$MESS ['T_IBLOCK_DESC_FNAME'] = "Название";
$MESS ['T_IBLOCK_DESC_FACT'] = "Дата начала активности";
$MESS ['T_IBLOCK_DESC_FSORT'] = "Сортировка";
$MESS ['T_IBLOCK_DESC_FTSAMP'] = "Дата последнего изменения";
$MESS ['T_IBLOCK_DESC_IBORD1'] = "Поле для первой сортировки новостей";
$MESS ['T_IBLOCK_DESC_IBBY1'] = "Направление для первой сортировки новостей";
$MESS ['T_IBLOCK_DESC_IBORD2'] = "Поле для второй сортировки новостей";
$MESS ['T_IBLOCK_DESC_IBBY2'] = "Направление для второй сортировки новостей";
$MESS ['T_IBLOCK_DESC_LIST_ID'] = "Код информационного блока";
$MESS ['T_IBLOCK_DESC_LIST_TYPE'] = "Тип информационного блока";
$MESS ['T_IBLOCK_DESC_LIST_CONT'] = "Количество новостей на странице";
$MESS ['T_IBLOCK_DESC_ACTIVE_DATE_FORMAT'] = "Формат показа даты";

$MESS ['NAME_FIELD'] = "Сделать именем элемента";
$MESS ['TITLE'] = "Заголовок формы";
$MESS ['SUCCESS_TEXT'] = "Текст, выводимый при успешном добавлении сообщения";
$MESS ['USE_CAPTCHA'] = "Использовать CAPTCHA";
$MESS ['PRINT_FIELDS'] = "Выводимые поля";
$MESS ['TEXT_REQUIRED'] = "Поле 'Сообщение' - обязательное";
$MESS ['TEXT_SHOW'] = "Показывать поле 'Сообщение'";
$MESS ['ALLOW_RESPONSE'] = "Включить возможность ответа";
$MESS ['SECTION_CODE'] = "Код раздела";
$MESS ['ELEMENT_ID'] = "ID элемента";
$MESS ['COLOR_SCHEME'] = "Цветовая схема";
$MESS ['ALWAYS_SHOW_PAGES'] = "Всегда показывать постраничную навигацию";

$MESS ['NAME'] = "Ф.И.О.";
$MESS ['EMAIL'] = "E-mail";
$MESS ['PHONE'] = "Телефонный номер";
$MESS ['MESSAGE'] = "Сообщение";
$MESS ['MESS_PER_PAGE'] = "Сообщений на страницу (не заполняйте чтобы вывести все)";
$MESS ['ACTIVE'] = "Активировать элемент после добавления";
$MESS ['EVENT_NAME'] = "Название почтового события";

$MESS ['CS_GREEN'] = "Зеленая";
$MESS ['CS_ICE'] = "Голубая";
$MESS ['CS_METAL'] = "Серая";
$MESS ['CS_PINK'] = "Розовая";
$MESS ['CS_RED'] = "Красная";
$MESS ['CS_YELLOW'] = "Желтая";
?>