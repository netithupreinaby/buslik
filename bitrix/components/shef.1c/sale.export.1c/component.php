...
require_once($_SERVER["DOCUMENT_ROOT"]."/local/components/shef.1c/lib/export.php");
...
else
{
	...
	elseif($_GET["mode"] == "query" || $_POST["mode"] == "query")
	{
		...
		CTimeZone::Disable();
		$arResultStat = CSaleExport2::ExportOrders2Xml(
	...
	elseif($_GET["mode"] == "import" && $_SESSION["BX_CML2_EXPORT"]["zip"] && strlen($_SESSION["BX_CML2_EXPORT"]["zip"]) > 1)
	{
		...
		$result = CSaleExport2::UnZip($_SESSION["BX_CML2_EXPORT"]["zip"], $_SESSION["BX_CML2_EXPORT"]["last_zip_entry"]);
		if($result===false)
		{
