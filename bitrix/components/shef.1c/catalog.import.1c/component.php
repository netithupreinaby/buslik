...
global $includeFileLangPath;
$includeFileLangPath = $_SERVER["DOCUMENT_ROOT"].'/bitrix/modules/iblock/classes/general/cml2.php';

require_once($_SERVER["DOCUMENT_ROOT"]."/local/components/shef.1c/lib/cml2xmlFile.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/local/components/shef.1c/lib/cml2.php");
...
elseif (($_GET["mode"] == "import") && $_SESSION["BX_CML2_IMPORT"]["zip"])
{
	...
	$result = CIBlockXMLFile2::UnZip($_SESSION["BX_CML2_IMPORT"]["zip"], $_SESSION["BX_CML2_IMPORT"]["last_zip_entry"]);
	if ($result===false)
	{
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	if ($NS["STEP"] < 1)
	{
		CIBlockXMLFile2::DropTemporaryTables();
		$strMessage = GetMessage("CC_BSC1_TABLES_DROPPED");
		$NS["STEP"] = 1;
	}
	elseif ($NS["STEP"] == 1)
	{
		if (CIBlockXMLFile2::CreateTemporaryTables())
		{
	...
	elseif ($NS["STEP"] == 3)
	{
		if (CIBlockXMLFile2::IndexTemporaryTables())
		{
	...
	elseif ($NS["STEP"] == 4)
	{
		$obCatalog = new CIBlockCMLImport2;
	...
	elseif ($NS["STEP"] == 7)
	{
		...
		{
			if (CIBlockXMLFile2::IsExistTemporaryTable())
			{
				$NS["DONE"]["ALL"] = CIBlockXMLFile2::GetCountItemsWithParent($NS["XML_ELEMENTS_PARENT"]);
			}
			else
			{
