...
class CIBlockCMLImport2
{
	...
	function Init(&$next_step, $files_dir = false, $use_crc = true, $preview = false, $detail = false, $use_offers = false, $use_iblock_type_id = false, $table_name = "b_xml_tree")
	{
		...
		$this->_xml_file = new CIBlockXMLFile2($table_name);
		...
		if ($this->next_step["lang"])
			$this->mess = IncludeModuleLangFile($includeFileLangPath, $this->next_step["lang"], true);
	...
	function CheckIfFileIsCML($file_name)
	{
		...
		if(file_exists($file_name) && is_file($file_name))
		{
			...
			if(is_resource($fp))
			{
				...
				{
					$mess = IncludeModuleLangFile($includeFileLangPath, $lang, true);
		...
		if (strlen($arProperty["USER_TYPE_SETTINGS"]["TABLE_NAME"]) <= 0)
		{
			...
			if (!$hlblock)
			{
				...
				$arFieldsName = array(
					...
					'UF_FULLDESCRIPTION' => array("N", "string"),
...
// self ///////////////
			if(array_key_exists($this->mess["IBLOCK_XML2_COEFF"], $arXMLElement))
			{
				$ratio = $this->ToFloat($arXMLElement[$this->mess["IBLOCK_XML2_COEFF"]]);
				$ratio = 0 >= $ratio ? 1 : $ratio;
				CCatalogMeasureRatio::add(array(
			...
			if(isset($arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]]))
			{
				$tmp = $this->convertBaseUnitFromXmlToPropertyValue($arXMLElement[$this->mess["IBLOCK_XML2_BASE_UNIT"]]);
