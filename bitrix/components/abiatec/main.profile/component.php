...
if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true)
	die();
$APPLICATION->SetAdditionalCSS("/bitrix/components/abiatec/main.profile/style.css", true);
...
$arParams['TEMPLATE_TYPE'] = ($arParams['TEMPLATE_TYPE']) ? $arParams['TEMPLATE_TYPE'] : 'LOGIN';
...
{
	// Check mandatory fields for each template will be added in next version
	if (is_array($arParams['MANDATORY_FIELDS'])){
		foreach ($arParams['MANDATORY_FIELDS'] as $mandatoryField){
			if (empty($_REQUEST[$mandatoryField])) {
				$strError .= GetMessage('EMPTY_' . $mandatoryField) . '<br />';
				$strErrorArray[$mandatoryField] .= GetMessage('EMPTY_' . $mandatoryField) . '<br />';
			}
		}
	}
	...
	if($strError == '')
	{
		...
		$arFields = array();
		switch ($arParams['TEMPLATE_TYPE']){
			case "LOGIN": $arFields["EMAIL"] = $_REQUEST['EMAIL'];
				$arFields['NEW_PASSWORD'] = $_REQUEST['NEW_PASSWORD'];
				break;
			case "DATA": $arFields["NAME"] = $_REQUEST['NAME'];
				$arFields['LAST_NAME'] = $_REQUEST['LAST_NAME'];
				$arFields['SECOND_NAME'] = $_REQUEST['SECOND_NAME'];
				$arFields['PERSONAL_BIRTHDAY'] = FormatDate("d.m.Y", getUTCtimestamp($_REQUEST['PERSONAL_BIRTHDAY']));
...
$arResult['arUser']['PERSONAL_BIRTHDAY'] = FormatDate("d F Y", MakeTimeStamp($arResult['arUser']['PERSONAL_BIRTHDAY']));
