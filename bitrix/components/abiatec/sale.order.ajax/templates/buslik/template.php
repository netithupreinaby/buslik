...
<section class="order-steps">
	<div class="row">
		<h1 class="cart-title"><?php echo $APPLICATION->ShowTitle(); ?></h1>
		<?php if (0) {?>
		<div class="alert alert-warning" role="alert">
			<p class="alert-text">Обращаем Ваше внимание, что обработку интернет-заказов на сайте и в колл-центре мы осуществляем с 10:00 по 21:00. Заказы, оформленные вне рабочее время будут обрабатываться с 10:00 следующего дня.</p>
		</div>
		<?php } ?>
		<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
				...
				<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
					<!--	MAIN BLOCK	-->
					<div class="col-lg-9 col-sm-7 bx-soa">
						<div class="left-column-wrap">
							...
							<? if (!isset($arParams['BASKET_POSITION']) || $arParams['BASKET_POSITION'] == 'before'): ?>
								<!--	BASKET ITEMS BLOCK	-->
								<div id="bx-soa-basket" data-visited="false" class="bx-soa-section bx-active order-step1">
									<div class="step-number">
										<span class="number cyan-bg">1</span>
										<span class="blue-text"><?php echo $arParams['MESS_BASKET_BLOCK_NAME']?></span>
									</div>
									<div class="bx-soa-section-content container-fluid"></div>
								</div>
							<? endif ?>
					...
					<!--	SIDEBAR BLOCK	-->
					<div id="bx-soa-total"  class="col-lg-3 col-sm-5 bx-soa-sidebar">
