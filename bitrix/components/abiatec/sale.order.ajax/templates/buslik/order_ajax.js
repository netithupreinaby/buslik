...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		{
			return BX.create('DIV', {props: {className: 'table bx-soa-section-content' + (!!notFluid ? '' : ' container-fluid')}});
		},
		...
		editActiveBasketBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				basketTable = BX.create('TBODY', {props: {className: 'bx-soa-item-table'}});
				...
				BX.bind(
					basketContent.querySelector('table.bx-soa-table-fade').firstChild,
					'scroll',
		...
		editFadeBasketBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				BX.bind(
					this.basketBlockNode.querySelector('table.bx-soa-table-fade').firstChild,
					'scroll',
					BX.proxy(this.basketBlockScrollCheckEvent, this)
				);
			}
		...
		//TODO do changes if headers will be shown
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			for (i = 0; i < this.result.GRID.HEADERS.length; i++)
			{
				...
				if (basketColumnIndex == 4 && this.result.GRID.HEADERS[i + 1])
				{
					otherColumns.push(BX.create('TD', {props: {className: 'bx-soa-item-nth-4p1'}}));
					basketColumnIndex = 0;
				}
			}
			...
			basketItemsNode.appendChild(
				BX.create('TR', {
					props: {className: 'table-line bx-soa-item-tr bx-soa-basket-info' + (index == 0 ? ' bx-soa-item-tr-first' : '')},
					children: cols})
			);
		...
		createBasketItemImg: function(data)
		{
			...
			return BX.create('TD', {
				props: {className: 'table-img bx-soa-item-img-block'},
				children: [logoNode]
			});
		},
		...
		createBasketItemContent: function(data)
		{
			...
			if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length)
				titleHtml = '<p class="product-name"><a href="' + data.DETAIL_PAGE_URL + '">' + titleHtml + '</a></p>';
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			{
				textNode.appendChild(BX.create('DIV', {props: {className: 'bx-price all'}, html: data.SUM_BASE_FORMATED}));
				if (parseFloat(data.DISCOUNT_PRICE) > 0)
				{
					textNode.appendChild(BX.create('SPAN', {
						props: {className: 'discount'},
						html: data.DISCOUNT_PRICE_DIFF
					}));
				}
			}
...
debugger;
			if (column.id == "SUM"){
				var additionalClass = "table-price clearfix ";
			}
			return BX.create('TD', {
				props: {className: additionalClass + 'bx-soa-item-td bx-soa-item-properties' + (toRight ? ' bx-text-right' : '')},
				children: [textNode]
			});
		},
