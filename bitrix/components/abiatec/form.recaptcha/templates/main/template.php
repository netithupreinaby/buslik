...
<p class="cart-title"><?=$arResult['arForm']['NAME']?></p>
...
?>
<?endif;?>
<input type="hidden" name="ELEMENT_ID" value="<?=$_SESSION['event_id']?>">
<?  
    ...
    ?>
            ...
            <?if(strstr($arQuestion["HTML_CODE"],'"inputtext"')){
              	$placeholder = $arQuestion["CAPTION"];
								...
								echo str_replace('<br>','','<div class="form-group"><label>'.$arQuestion['CAPTION'].$reqSign.'</label>'.str_replace('inputtext','form-control'.$classValidate,$arQuestion["HTML_CODE"]).'</div>');
                ...
                $placeholder = $arQuestion["CAPTION"];
                 ...
                 $placeholder = 'placeholder="'.$placeholder.'"';
                echo '</ul></div><div class="col-sm-6 col-sm-offset-0 col-xs-12 col-xs-offset-0"><ul class="popup_fields">';
                $textarea =  str_replace('<textarea','<textarea '.$placeholder,$arQuestion["HTML_CODE"]);
	...
	<?if($arResult['captcha_error']){?>
	<p class="error captcha-error"><?=$arResult['captcha_error']?>
	<?}?>

	<div class="form-group"> 
		<input <?=(intval($arResult["F_RIGHT"]) < 10 ? "disabled=\"disabled\"" : "");?> type="submit" name="web_form_submit" value="<?=htmlspecialcharsbx(strlen(trim($arResult["arForm"]["BUTTON"])) <= 0 ? GetMessage("FORM_ADD") : $arResult["arForm"]["BUTTON"]);?>" class="btn btn-blue btn-white-color btn btn-danger" />
