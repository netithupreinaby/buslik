...
/*************************************************************************************************/
	...
	if (strlen($arResult["ERROR"]) <= 0) {
		...
		if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && (strlen($_REQUEST["web_form_submit"]) > 0 || strlen($_REQUEST["web_form_apply"]) > 0)) {
			...
			if (!$resp->isSuccess()) {
				foreach ($resp->getErrorCodes() as $code) {
					$arResult["FORM_ERRORS"] .= " <br />captcha error";
					$arResult["captcha_error"]  =  'отметьте captcha';
		...
		*/
		...
		/*	if ($arParams['WEB_FORM_ID'] == '2' && !$arResult["FORM_ERRORS"]) {
                ...
                $arRes = $rsRes->Fetch();
                $asdf = CFormResult::GetDataByID($arRes['ID']);
		...
		if ($arParams['WEB_FORM_ID'] == '1' && !$arResult["FORM_ERRORS"]) {
