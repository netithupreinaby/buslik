		...
		for (i = 0; i < arColumns.length; i++)
		{
			switch (arColumns[i])
			{
				...
				case 'PRICE':
					...
					discountDiffPrice = arItem['DISCOUNT_PRICE_DIFF_FORMATED'];
...
debugger;
					...
					oCellPrice.innerHTML += '<span id="old_price_' + arItem['ID'] + '">' + discountDiffPrice + '</span>';
	...
	if (!!res.BASKET_DATA)
	{
		for (id in res.BASKET_DATA.GRID.ROWS)
		{
			if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id))
			{
				...
				if (BX('old_price_' + id))
					BX('old_price_' + id).innerHTML = (item.DISCOUNT_PRICE_DIFF > 0) ? item.DISCOUNT_PRICE_DIFF_FORMATED : '';
...
debugger;
		if (BX('allSum_order_FORMATED'))
			BX('allSum_order_FORMATED').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;');
...
function skuPropClickHandler(e)
{
	...
	if (!!target && target.hasAttribute('data-value-id'))
	{
		...
		BX.ajax({
			url: '/bitrix/components/abiatec/sale.basket.basket/ajax.php',
			method: 'POST',
			data: postData,
			dataType: 'json',
