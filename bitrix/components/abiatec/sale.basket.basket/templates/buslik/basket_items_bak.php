		...
		<table class="table" id="basket_items">
			<thead>
				<tr>
					...
					<th colspan="2">Товары (<span><?=count(5);?></span>)</th>
					<th>Количество</th>
					<th>Цена за 1шт.</th>
					<th>Сумма</th>
					<th></th>
					...
					foreach ($arResult["GRID"]["HEADERS_MY"] as $id => $arHeader){
						...
						if ($arHeader["id"] == "NAME"):
						?>
							<th class="item" colspan="2" id="col_<?=$arHeader["id"];?>">
						<?
			...
			<tbody>
				...
				?>
				<?//printr($arItem)?>
						<tr class="table-line" id="<?=$arItem["ID"]?>">
						<td class="testblock"></td>
						<td class="table-img">
								<?
						...
						<td class="table-item" colspan=2>
							<div class="">
								...
								<p class="item-brand">
									<a href="#" class="brand-name">ADAMEX</a> 
									<span class="iten-number">Артикул: 36587798</span>
								</p>
								<p class="item-parameter">Color: 
									<span class="item-color"></span> size: 
									<span class="item-size">36</span> height: 
									<span class="item-height">180</span>
								</p>
							</div>
							
							<div class="number clearfix">
								<span class="arrow-left counter-button" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'down', <?=$useFloatQuantityJS?>);"></span>
								<?
										...
										>
								<span class="arrow-right counter-button" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'up', <?=$useFloatQuantityJS?>);" ></span>
								<input type="hidden" id="QUANTITY_<?=$arItem['ID']?>" name="QUANTITY_<?=$arItem['ID']?>" value="<?=$arItem["QUANTITY"]?>" />
							</div>
										
						</td>
						
						<td data-price="<?=$arItem['DISCOUNT_PRICE_PERCENT']?>" class="table-price clearfix">
							...
							<?/*if($arItem['DISCOUNT_PRICE_PERCENT']>0){?>
								<div data=""><?=$arItem['FULL_PRICE_FORMATED']?></div>
								<span class="discount">-34.90 руб.</span>
							<?}else{?>
								<div data=""><?=$arItem['PRICE_FORMATED']?></div>
							<?}*/?>
						</td>
						<td class="table-total">
							...
							<div class="bonus-block">
								<span class="bonus">
									<img src="img/icons/bonus.png" alt="">
								</span>
