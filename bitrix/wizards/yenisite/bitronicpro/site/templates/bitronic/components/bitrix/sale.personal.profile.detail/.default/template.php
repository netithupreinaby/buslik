...
<?include $_SERVER["DOCUMENT_ROOT"].SITE_TEMPLATE_PATH.'/include/cache_time_debug.php';?>
	...
	<?	/*	NEW ERROR MESSAGE	*/
			$ERROR = preg_split("/<br\s\/>/",$arResult["ERROR_MESSAGE"]);
	...
	<input type="hidden" name="ID" value="<?=$arResult["ID"]?>">
                <div class="profile_item_cont">
                    <div class="profile_item">
                            <?/*<h2 class="prifile"><?= str_replace("#ID#", $arResult["ID"], GetMessage("SPPD_PROFILE_NO")) ?></h2>*/?>
                            <div class="form-item">
                            ...
                            <div class="form-item">
								...
								<input type="text" name="NAME" value="<?echo $arResult["NAME"];?>" class="txt" />
                            </div><!--.form-item-->
		...
		foreach($arResult["ORDER_PROPS"] as $val)
		{
			if(!empty($val["PROPS"]))
			{
				foreach($val["PROPS"] as $vval)
				{
					...
					?>
					<div class="form-item">
						<label><?=$vval["NAME"]?>:<?if ($vval["REQUIED"]=="Y")
							{
							?><span class="req">*</span><?
							}
							?></label>
		...
		?>		
                    ...
                    <div class="form-submit">
						...
						<input type="hidden" id="reset" name="reset" value="">
                           ...
                           <button onclick="$('#save').attr('value', 'Y');" class="button1"><?echo GetMessage("SALE_SAVE") ?></button>
                           <button onclick="$('#apply').attr('value', 'Y');" class="button1"><?=GetMessage("SALE_APPLY")?></button>
                           <button onclick="$('#reset').attr('value', 'Y');" class="button1"><?echo GetMessage("SALE_RESET")?></button>
                    </div><!--.form-submit-->
				</div><!--.profile_item-->
                </div><!--.profile_item_cont-->
				</form>
				...
				foreach($val["PROPS"] as $vval)
				{
					...
					<tr>
						...
						<td width="50%">
							<?if ($vval["TYPE"]=="CHECKBOX"):?>
								<input type="hidden" name="<?=$name?>" value="">
								<input type="checkbox" class="checkbox" name="<?=$name?>" value="Y"<?if ($currentValue=="Y" || !isset($currentValue) && $vval["DEFAULT_VALUE"]=="Y") echo " checked";?>>
							<?elseif ($vval["TYPE"]=="TEXT"):?>
								<input type="text" class="txt" size="<?echo (IntVal($vval["SIZE1"])>0)?$vval["SIZE1"]:30; ?>" maxlength="250" value="<?echo (isset($currentValue)) ? $currentValue : $vval["DEFAULT_VALUE"];?>" name="<?=$name?>">
							<?elseif ($vval["TYPE"]=="SELECT"):?>
								<select name="<?=$name?>" class="select" size="<?echo (IntVal($vval["SIZE1"])>0)?$vval["SIZE1"]:1; ?>">
									<?foreach($vval["VALUES"] as $vvval):?>
										<option value="<?echo $vvval["VALUE"]?>"<?if ($vvval["VALUE"]==$currentValue || !isset($currentValue) && $vvval["VALUE"]==$vval["DEFAULT_VALUE"]) echo " selected"?>><?echo $vvval["NAME"]?></option>
									<?endforeach;?>
								</select>
