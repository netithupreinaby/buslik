...
<div class="bx_item_set_hor_container_big <? echo $templateData['TEMPLATE_CLASS']; ?>">
	...
	<div class="bx_item_set_hor">
		<div class="bx_item_set_hor_item plus" style="width:<?=$blockWidth?>%;" data-price="<?=$arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"]?>" data-old-price="<?=$arResult["ELEMENT"]["PRICE_VALUE"]?>" data-discount-diff-price="<?=$arResult["ELEMENT"]["PRICE_DISCOUNT_DIFFERENCE_VALUE"]?>">
			...
			<a class="bx_item_set_linkitem" href="<?=$arResult["ELEMENT"]["DETAIL_PAGE_URL"]?>"><span class="bx_item_set_name"><?=$arResult["ELEMENT"]["NAME"]?></span> <br /><br />
		...
		<?foreach($arResult["SET_ITEMS"]["DEFAULT"] as $key => $arItem):?>
			<div class="bx_item_set_hor_item <?if ($key<$countDefSetItems-1) echo "plus"; else echo "equally"?> bx_default_set_items"
				...
				<a class="bx_item_set_linkitem" href="<?=$arItem["DETAIL_PAGE_URL"]?>"><span class="bx_item_set_name"><?=$arItem["NAME"]?></span><br /><br />
		...
		<div class="bx_item_set_hor_item result">
			...
			<a href="javascript:void(0)" onclick="catalogSetDefaultObj_<? echo $intElementID; ?>.Add2Basket(this);" class="<?//bx_bt_button_type_2 bx_medium?>button2"><?=GetMessage("CATALOG_SET_BUY")?></a>
		</div>

	</div>
	<a class="bx_item_set_creator_link" href="javascript:void(0)" rel="nofollow" onclick="OpenCatalogSetPopup('<?=$intElementID?>');"><?=GetMessage("CATALOG_SET_CONSTRUCT")?></a>
</div>
...
$popupParams["MESS"] = array(
	...
	"CATALOG_SET_ADD2BASKET" => GetMessage("CATALOG_SET_ADD2BASKET"),
	...
	function OpenCatalogSetPopup(element_id)
	{
		...
		BX.CatalogSetConstructor.popup = BX.PopupWindowManager.create("CatalogSetConstructor_"+element_id, null, {
			...
			events: {
				...
				{
					var popup = this;
					$(this.closeIcon).html('&#205;').css({'background':'none'});
					$(this.overlay.element).off('click').on('click', function(){popup.close();});
					BX.ajax.post(
						...
						BX.delegate(function(result)
						{
							...
							var popupTop = scrollTop + (window.innerHeight - BX("CatalogSetConstructor_"+element_id).offsetHeight)/2;
