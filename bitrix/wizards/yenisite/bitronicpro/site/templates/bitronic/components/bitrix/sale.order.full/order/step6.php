...
<div class="order_accept">	
	<div class="order_content">
		...
		<?= str_replace("#ORDER_DATE#", $arResult["ORDER"]["DATE_INSERT_FORMATED"], str_replace("#ORDER_ID#", $arResult["ORDER"]["ACCOUNT_NUMBER"], GetMessage("STOF_ORDER_CREATED_DESCR"))); ?> <a href="/personal/orders.php?ID=<?=$arResult["ORDER"]["ID"]?>"><?=GetMessage('GOTO')?></a><br /><br />
		
		<?if ($arResult["PAY_SYSTEM"]["NEW_WINDOW"] == "Y")
		{
		?>
			<script language="JavaScript">
				window.open('<?=$arParams["PATH_TO_PAYMENT"]?>?ORDER_ID=<?= $arResult["ORDER"]["ID"] ?>');
			</script>
			<?= str_replace("#LINK#", $arParams["PATH_TO_PAYMENT"]."?ORDER_ID=".$arResult["ORDER"]["ID"], GetMessage("STOF_ORDER_PAY_WIN")) ?>
		<?
		}
		else
		{
