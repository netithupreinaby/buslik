function catalogSetConstructDefault(arSetIds, ajax_path, ajax_basket_path, price_currency, lid, element_id, detail_img, items_ratio)
{
	this.arSetIDs = arSetIds;
	this.ajax_path = ajax_path;
	this.ajax_basket_path = ajax_basket_path;
...
catalogSetConstructDefault.prototype.Add2Basket = function(basketButton)
{
	var detail_img = this.detail_img;
	var element_id = this.element_id;
	BX.ajax.post(
		this.ajax_basket_path,
		{
			sessid: BX.bitrix_sessid(),
			action: 'ADD2BASKET',//'catalogSetAdd2Basket',
			//set_ids: this.arSetIDs,
			id: this.arSetIDs,
			//lid: this.lid,
			SITE_ID: this.lid,
			//iblockId: BX.message('setIblockId'),
			iblock_id: BX.message('setIblockId'),
			setOffersCartProps: BX.message('setOffersCartProps'),
			itemsRatio: this.items_ratio,
			action_add2b: 'popup_basket'
		},
		function(result)
		{
			//showCatalogSetAdd2BasketPopup(detail_img , element_id);
			if (result.indexOf('<!-- errors -->') != -1) {
				var arData = result.split('<!-- errors -->');
				jGrowl(arData[1], 'error');
				result = arData[0];
				if (result.indexOf('<div class="yen-bs-count yen-bs-up">') != -1) {
					$('.yen-bs-box').html(result);
				}
			} else {
				$(basketButton)
				.closest('div.bx_item_set_hor')
				.find('div.bx_item_set_img_container a').each(function(){
					var what = $(this);
					var to = $('.yen-bs-box');
					var pic = what.clone();
					
					pic.css({'z-index':'1000', 'position':'absolute', 'left': $(what).offset().left + 'px', 'top': $(what).offset().top + 'px',
						'background-size':'98% auto', 'background-repeat':'no-repeat', 'background-position':'center'});
					pic.height(what.innerHeight());
					pic.width(what.width());
...
function catalogSetConstructPopup(ItemsCount, ItemsWidth, Currency, DefaultItemPrice, DefaultItemDiscountPrice, DefaultItemDiscountDiffPrice, ajaxPath, ajaxBasketPath, setIds, lid, element_id, items_ratio, detail_img)
{
	...
	this.ajaxBasketPath = ajaxBasketPath;
...
//			iblockId: BX.message('setIblockId'),
		...
		function(result)
		{
			...
			if (result.indexOf('<!-- errors -->') != -1) {
				...
				$('div.bx_kit_one_section div.bx_kit_img_container').each(function(){
					var what = $(this);
					var to = $('.yen-bs-box');
					var pic = what.clone();
