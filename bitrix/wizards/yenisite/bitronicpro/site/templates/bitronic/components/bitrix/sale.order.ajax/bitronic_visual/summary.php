...
<tbody>
	...
	foreach($arResult["BASKET_ITEMS"] as $arBasketItems)
	{
		...
		<tr>
			<td>
				<?
				$image = yenisite_GetPicSrc($arBasketItems);
				$image_src = CFile::GetPath($image);
				if (CModule::IncludeModule('yenisite.resizer2')) 
					$pathResizeImage = CResizer2Resize::ResizeGD2($image_src, 3);
				else
				{
					$ResizeParams = array('width' => $arParams['DISPLAY_IMG_WIDTH'], 'height' => $arParams['DISPLAY_IMG_HEIGHT']);
					$ResizeImage = CFile::ResizeImageGet($image, $ResizeParams,  BX_RESIZE_IMAGE_PROPORTIONAL, true);
						
					$pathResizeImage = $ResizeImage['src'] ;
				}
				echo CFile::ShowImage($pathResizeImage, $arParams["DISPLAY_IMG_WIDTH"], $arParams["DISPLAY_IMG_HEIGHT"], "border=0", "", false);
				/*
			...
			<td class="price"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arBasketItems["PRICE_FORMATED"]);?></td>
		</tr>
		<?
	}
	?>
	...
	<tr>
		...
		<td class="price"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arResult["ORDER_PRICE_FORMATED"]);?></td>
	</tr>
	<?
	if (doubleval($arResult["DISCOUNT_PRICE"]) > 0)
	{
		?>
		<tr>
			<td colspan="6" class="itog"><?=GetMessage("SOA_TEMPL_SUM_DISCOUNT")?><?if (strLen($arResult["DISCOUNT_PERCENT_FORMATED"])>0):?> (<?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arResult["DISCOUNT_PERCENT_FORMATED"]);?>)<?endif;?>:</td>
			<td class="price"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>',  $arResult["DISCOUNT_PRICE_FORMATED"]);?></td>
		</tr>
		<?
	}
	if(!empty($arResult["arTaxList"]))
	{
		foreach($arResult["arTaxList"] as $val)
		{
			...
			<tr>
				...
				<td class="price"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>', $val["VALUE_MONEY_FORMATED"]);?></td>
			</tr>
			<?
		}
	}
	if (doubleval($arResult["DELIVERY_PRICE"]) > 0)
	{
		...
		<tr>
			...
			<td class="price"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>', $arResult["DELIVERY_PRICE_FORMATED"]);?></td>
		</tr>
		<?
	}
	if (strlen($arResult["PAYED_FROM_ACCOUNT_FORMATED"]) > 0)
	{
		...
		<tr>
			...
			<td class="price"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>', $arResult["PAYED_FROM_ACCOUNT_FORMATED"]);?></td>
		</tr>
		<?
	}
	?>
	<tr class="last">
		...
		<td class="price"><?=str_replace(GetMessage('RUB_REPLACE'), '<span class="rubl">'.GetMessage('RUB').'</span>', $arResult["ORDER_TOTAL_PRICE_FORMATED"]);?></td>
	</tr>
</tfoot>
</table>
