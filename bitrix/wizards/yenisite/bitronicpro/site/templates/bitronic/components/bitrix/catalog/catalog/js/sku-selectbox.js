	...
	if (-1 < index)
	{
		if (!!this.obPict)
		{
			if(this.viewMode == 'table')
			{
				...
				{
					BX.adjust(BX.findParent(this.obPict[0], {'tag':'a'}), {attrs: {rel: this.offers[index].PREVIEW_PICTURE}});
					BX.adjust(this.obPict[0], {attrs: {src: this.offers[index].PREVIEW_PICTURE_SMALL}});
				}
				else
				{
					BX.adjust(BX.findParent(this.obPict[0], {'tag':'a'}), {attrs: {rel: this.defaultPict.pict}});
					BX.adjust(this.obPict[0], {attrs: {src: this.defaultPict.smallPict}});
				}
			}
			else if(this.viewMode == 'block')
			{
				if (!!this.offers[index].PREVIEW_PICTURE)
				{
		...
		if (!!this.obBuyBtn && this.offers[index].ID > 0)
		{
			...
			{
				obData = {
					attrs: { 'value': this.offers[index].ID}
				};
			}
			else
			{
