<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
				...
				foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
				{
					...
					<p class="auto_delivery">
						...
						<?endforeach;?>
					<?else:?>
						<p class="custom_delivery"><input type="radio" class="radio" id="ID_DELIVERY_ID_<?= $arDelivery["ID"] ?>" name="<?=$arDelivery["FIELD_NAME"]?>" value="<?= $arDelivery["ID"] ?>"<?if ($arDelivery["CHECKED"]=="Y") echo " checked";?>>
						<label for="ID_DELIVERY_ID_<?= $arDelivery["ID"] ?>"><?= $arDelivery["NAME"] ?></label>
						<div class="custom_delivery_profile">
							<?if(!empty($arDelivery["PRICE_FORMATED"])):?>
							<?=GetMessage("SALE_DELIV_PRICE");?>: <b><?=$arDelivery["PRICE_FORMATED"]?></b>
							<?endif;?>
						</div>
						</p>
					<?endif;?>
