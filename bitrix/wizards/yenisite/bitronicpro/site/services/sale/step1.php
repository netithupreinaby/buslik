...
if(COption::GetOptionString("store", "wizard_installed", "N", WIZARD_SITE_ID) != "Y" || WIZARD_INSTALL_DEMO_DATA)
{
	...
	if(!$dbPerson->Fetch())//if there are no data in module
	{
		...
		if($personType["fiz"] == "Y")
		{
			...
			/* === Warning! Do not change next two props order and codes === */
			$arProps[] = Array(
					"PERSON_TYPE_ID" => $arGeneralInfo["personType"]["fiz"],
			...
			/* === Warning! Do not change previous two props order and codes === */
			$arProps[] = Array(
					"PERSON_TYPE_ID" => $arGeneralInfo["personType"]["fiz"],
		...
		foreach($arProps as $prop)
		{
			...
			if ($prop["PERSON_TYPE_ID"] == $arGeneralInfo["personType"]["ur"]
			&&  $prop["CODE"] == 'LOCATION') {
				//City as alternative location for ur person
				$prop['INPUT_FIELD_LOCATION'] = $arGeneralInfo["propCodeID"]['CITY'];
			}

			$id = CSaleOrderProps::Add($prop);

			if ($prop["PERSON_TYPE_ID"] == $arGeneralInfo["personType"]["fiz"]
			&&  $prop['CODE'] == 'CITY') {
				//City as alternative location for fiz person
				CSaleOrderProps::Update($arGeneralInfo["propCodeID"]['LOCATION'], array('INPUT_FIELD_LOCATION' => $id));
			}
